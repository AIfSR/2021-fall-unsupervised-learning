...
class SingleEpisode2DCompareTrajectories (ComparePlotsBase):
    ...
    def display_plots(self, graphParameters:List[GraphParameters], categories:List[Tuple[str,List[Episodes]]]) -> None:
        """Displays plots comparing single episode values of a feature for each category"""
    ...
    def _plot_features(self, categories:List[Tuple[str,List[Episodes]]], yFeatureCreator:FeatureCreatorBase, xFeatureCreator:FeatureCreatorBase, yLabel:str, xLabel:str, featuresToSingleVal:FeatureToSingleValBase) -> None:
        """Takes the two feature creator bases and plots the single episode values 
    ...
    def _create_graph_and_histogram_sections(self, categories:List[Tuple[str,List[Episodes]]]) -> Tuple[Axes, Dict]:
        ...
        histDict = {}
        left = SingleEpisode2DCompareTrajectories.LEFT
        bottom = SingleEpisode2DCompareTrajectories.BOTTOM
        width = SingleEpisode2DCompareTrajectories.WIDTH
        height = SingleEpisode2DCompareTrajectories.HEIGHT
    ...
    def _create_hist_section(self, axis:str, i:int, histHeight:float, ax_scatter:Axes) -> Axes:
        ...
        width = SingleEpisode2DCompareTrajectories.WIDTH
        height = SingleEpisode2DCompareTrajectories.HEIGHT
        spacing = SingleEpisode2DCompareTrajectories.SPACING
        if axis == "X":
    ...
    def _plot_2D_data_and_hists(self, categories:List[Tuple[str,List[Episodes]]], ax_scatter:Axes, histogramDict:Dict,  xFeatureCreator:FeatureCreatorBase, yFeatureCreator:FeatureCreatorBase, featuresToSingleVal) -> Dict:
        """Plots the data passed in in two dimensions on the scatterplot, and the histograms"""
        histBinsDict = {}
        episodesPlotted = []
        for name, allFilesEpisodes in categories:
            xEpisodesAverages = []
            yEpisodesAverages = []
            for trajectoryEpisodes in allFilesEpisodes:
                xFeature = xFeatureCreator.get_features(trajectoryEpisodes)
                yFeature = yFeatureCreator.get_features(trajectoryEpisodes)
                xAvgOfFeature = featuresToSingleVal.get_val(xFeature)
                yAvgOfFeature = featuresToSingleVal.get_val(yFeature)
                xEpisodesAverages.append(xAvgOfFeature)
                yEpisodesAverages.append(yAvgOfFeature)
            
            s = ax_scatter.scatter(xEpisodesAverages, yEpisodesAverages, label=name)
            episodesPlotted.append((name, xEpisodesAverages, yEpisodesAverages, s.get_ec()))
        ...
        for name, xEpisodesAverages, yEpisodesAverages, color in episodesPlotted:
            xHistAxis = histogramDict[name + " X"]
            yHistAxis = histogramDict[name + " Y"]
            histBinsDict[name + " X"] = xHistAxis.hist(xEpisodesAverages, color = color, bins=ax_scatter.get_xticks(), range=ax_scatter.get_xlim())
            histBinsDict[name + " Y"] = yHistAxis.hist(yEpisodesAverages, orientation='horizontal', color = color, bins=ax_scatter.get_yticks(), range=ax_scatter.get_ylim())
    ...
    def _get_graph_title(self, categories:List[Tuple[str,List[Episodes]]], yLabel:str, xLabel:str) -> str:
