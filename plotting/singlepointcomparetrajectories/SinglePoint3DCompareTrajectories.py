...
class SinglePoint3DCompareTrajectories (ComparePlotsBase):
    ...
    def display_plots(self, graphParameters:List[GraphParameters], categories:List[Tuple[str,List[Points]]]) -> None:
        ...
        for graphParameter in graphParameters:
            plt.close()
            self._plot_features(categories, graphParameter.zFeatureCreator, graphParameter.yFeatureCreator, graphParameter.xFeatureCreator, graphParameter.zLabel, graphParameter.yLabel, graphParameter.xLabel, graphParameter.featuresToSingleVal)
            plt.show()

    def _plot_features(self, categories:List[Tuple[str,List[Points]]], zFeatureCreator:FeatureCreatorBase, yFeatureCreator:FeatureCreatorBase, xFeatureCreator:FeatureCreatorBase, zLabel:str, yLabel:str, xLabel:str, featuresToSingleVal:FeatureToSingleValBase) -> None:
        ...
        ax_scatter = self._create_3Dgraph(categories)
        ...
        ax_scatter = self._plot_3D_data(categories, ax_scatter,zFeatureCreator, xFeatureCreator, yFeatureCreator, featuresToSingleVal)
        # self._set_hist_size_and_labels(histBinsDict, histogramDict, ax_scatter)
        ax_scatter.set_zlabel(zLabel)
        ax_scatter.set_ylabel(yLabel)
        ax_scatter.set_xlabel(xLabel)
        fig.suptitle(self._get_graph_title(categories, zLabel, yLabel, xLabel))
        fig.legend(prop={"size":20})

    def _create_3Dgraph(self, categories:List[Tuple[str,List[Points]]]) -> Axes:
        ...
        # histDict = {}
        # left = SinglePoint3DCompareTrajectories.LEFT
        # bottom = SinglePoint3DCompareTrajectories.BOTTOM
        # width = SinglePoint3DCompareTrajectories.WIDTH
        # height = SinglePoint3DCompareTrajectories.HEIGHT
    ...
    #     width = SinglePoint3DCompareTrajectories.WIDTH
    #     height = SinglePoint3DCompareTrajectories.HEIGHT
    #     spacing = SinglePoint3DCompareTrajectories.SPACING
    #     if axis == "X":
    ...
    def _plot_3D_data(self, categories:List[Tuple[str,List[Points]]], ax_scatter:Axes, xFeatureCreator:FeatureCreatorBase, yFeatureCreator:FeatureCreatorBase, zFeatureCreator:FeatureCreatorBase, featuresToSingleVal) -> Axes:
        """Plots the data passed in in two dimensions on the scatterplot, and the histograms"""
        # histBinsDict = {}
        pointsPlotted = []
        for name, allFilesPoints in categories:
            ...
            for trajectoryPoints in allFilesPoints:
                xFeature = xFeatureCreator.get_features(trajectoryPoints)
                yFeature = yFeatureCreator.get_features(trajectoryPoints)
                zFeature = zFeatureCreator.get_features(trajectoryPoints)
                xAvgOfFeature = featuresToSingleVal.get_val(xFeature)
                yAvgOfFeature = featuresToSingleVal.get_val(yFeature)
                zAvgOfFeature = featuresToSingleVal.get_val(zFeature)
                xPointsAverages.append(xAvgOfFeature)
                yPointsAverages.append(yAvgOfFeature)
                zPointsAverages.append(zAvgOfFeature)
            s = ax_scatter.scatter(xPointsAverages, yPointsAverages, zPointsAverages, label=name)
            pointsPlotted.append((name, xPointsAverages, yPointsAverages, zPointsAverages, s.get_ec()))

        # ax_scatter.set_xticks(ax_scatter.get_xticks())
        # ax_scatter.set_yticks(ax_scatter.get_yticks())
        # ax_scatter.set_zticks(ax_scatter.get_zticks())
    ...
    def _get_graph_title(self, categories:List[Tuple[str,List[Points]]], zLabel:str, yLabel:str, xLabel:str) -> str:
        """Gets the name of the graph."""
        title = ""
        for name, _ in categories:
            title += name + ", "
        
        title += str(zLabel) + " vs. " + str(yLabel) + " vs. " + str(xLabel) + " comparison"
        return title
